...
function Client(){
                ...
                <div className="card">
                    ...
                    <div className="p-2">
                        {Object.entries(el.accounts).map((accountType)=>(Object.entries(accountType[1]).map(accounts=>(
                            <div className="p-2 border mt-1" style={{backgroundColor: accounts[1]===0? "#FF3333":"white",borderRadius:"15px",fontSize:"1.2rem",textAlign:"start"}}>
    ...
    <form className="justify-content-center my-5 d-flex col-9 mx-auto flex-column">
        ...
        <select className="form-control" name="accountType">
            ...
            {userData.map(el=>(<option>
                {el.firstname+" "+el.lastname}
            </option>))}
        </select>
        <label className="form-label">Wprowadz kwotę</label>
        <input type="number" className="form-control" placeholder="Kwota" name="amount"/>
        ...
        <select className="form-control" name="subaccountType">
            <option disabled selected>Typ konta</option>
            <option>USD</option>
            <option>EURO</option>
            <option>PLN</option>
        ...
        <div className="d-flex p-0 mt-3">
            <button className="btn btn-primary">wpłać</button>
            <button className="btn btn-primary mx-2">wypłać</button>
        </div>
   </form>
  </main>)
}
